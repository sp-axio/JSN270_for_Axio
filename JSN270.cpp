...
JSN270::JSN270(HardwareSerial *serial)
{
	instance = this;
	this->serial = serial;
...
{
	boolean found = find((char *)"CONNECTED 0");
	if (found) {
		return true;
	}
	return false;
...
int JSN270::send(const uint8_t *data, int len, int timeout)
{
    ...
    while (write_bytes < len) {
        if (write(data[write_bytes]) == 1) {
            ...
            if (write_error) {
                if ((millis() - start_millis) > (unsigned long)timeout) {
...
void JSN270::send(const char *data, int timeout)
{
...
boolean JSN270::ask(const char *q, const char *a, int timeout)
{
    ...
    if (a != NULL) {
        ...
        if (!found) {
            ...
            if ((end - start) < (unsigned long)timeout) {
                DBG("\r\n");
                DBG(q);
                DBG("\r\nTry to find: ");
                DBG(a);
